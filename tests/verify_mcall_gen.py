...
def main():
    ...
    project_path = Path("/Users/qlwang/Desktop/ImportEB_1")
    
    if not project_path.exists():
        logging.error(f"Project path does not exist: {project_path}")
        return
        
    logging.info(f"Importing project from {project_path} ...")
    project, loaded, failed = workspace.import_eb_project(project_path, chip_name="THA6206_LFBGA292")
    if not project:
        logging.error("Failed to import EB project.")
        return
        
    # Get Os module
    os_manager = project.get_manager("Mcall")
    os_config = os_manager.configuration if os_manager else None
    if not os_config:
        logging.error("Os configuration not found in workspace!")
        return
    ...
    template_dir = project_path / "MCAL_R440_FuSa" / "templates"
