...
[!LOOP "node:order(../../CanHardwareObject/*, 'node:value(node:ref(./CanControllerRef)/CanHardwareChannel)')"!][!//
          [!/* Only process the Hoh matched the same activated controller */!][!//
[!IF "node:ref(./CanControllerRef)/CanControllerActivation = 'true' and $CurrentController = node:value(node:ref(./CanControllerRef)/CanHardwareChannel)"!][!//
